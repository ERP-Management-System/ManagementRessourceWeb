...
import {  Matiere, ModeReglement, Taille } from 'src/app/parametrageCenral/domaine/ParametrageCentral';
...
@Component({
  ...
  styleUrl: './taille.component.css', providers: [ConfirmationService, MessageService]
})
...
export class TailleComponent {
  ...
  DeleteTaille(code: any) {
    this.param_achat_service.DeleteTaille(code).pipe(
  ...
  public onOpenModal(mode: string) {
    ...
    if (mode === 'add') {
      ...
      this.formHeader = "Nouveau Taille" 
      this.onRowUnselect(event); 
      this.clearSelected();
      this.actif = false;
      this.visible = false;
    ...
    if (mode === 'Print') {
      ...
      this.formHeader = "Imprimer Liste Taille"
  ...
  PostTaille() {
    ...
    if (!this.designationAr || !this.designationLt || !this.codeSaisie) {
      ...
      if (this.code != null) {
        ...
        this.param_achat_service.UpdateTaille(body).pipe(
          catchError((error: HttpErrorResponse) => {
            let errorMessage = '';
            if (error.error instanceof ErrorEvent) {
            } else {
        ...
        this.param_achat_service.PostTaille(body).pipe(
          catchError((error: HttpErrorResponse) => {
            let errorMessage = '';
