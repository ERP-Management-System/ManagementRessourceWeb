...
import {  Matiere, ModeReglement, Taille, TypeMatiere, Unite } from 'src/app/parametrageCenral/domaine/ParametrageCentral';
...
@Component({
  ...
  styleUrl: './unite.component.css', providers: [ConfirmationService, MessageService]
})
export class UniteComponent {
  ...
  DeleteUnite(code: any) {
    this.param_achat_service.DeleteUnite(code).pipe(
  ...
  public onOpenModal(mode: string) {
    ...
    if (mode === 'Print') {
      ...
      this.formHeader = "Imprimer Liste Unite"
  ...
  PostUnite() {
    ...
    if (!this.designationAr || !this.designationLt || !this.codeSaisie) {
      ...
      if (this.code != null) {
        ...
        this.param_achat_service.UpdateUnite(body).pipe(
          catchError((error: HttpErrorResponse) => {
            let errorMessage = '';
            if (error.error instanceof ErrorEvent) {
            } else {
        ...
        this.param_achat_service.PostUnite(body).pipe(
          catchError((error: HttpErrorResponse) => {
            let errorMessage = '';
