...
import {  Matiere, ModeReglement, Taille, TypeMatiere } from 'src/app/parametrageCenral/domaine/ParametrageCentral';
...
@Component({
  selector: 'app-type-matiere',
  templateUrl: './type-matiere.component.html',
  styleUrl: './type-matiere.component.css', providers: [ConfirmationService, MessageService]
})
export class TypeMatiereComponent {
  ...
  selectedTypeMatiere!: TypeMatiere; 
  ...
  DeleteTypeMatiere(code: any) {
    this.param_achat_service.DeleteTypeMatiere(code).pipe(
  ...
  public onOpenModal(mode: string) {
    ...
    if (mode === 'Print') {
      ...
      this.formHeader = "Imprimer Liste Type Matiere"
  ...
  PostTypeMatiere() {
    ...
    if (!this.designationAr || !this.designationLt || !this.codeSaisie) {
      ...
      if (this.code != null) {
        ...
        this.param_achat_service.UpdateTypeMatiere(body).pipe(
          catchError((error: HttpErrorResponse) => {
            let errorMessage = '';
            if (error.error instanceof ErrorEvent) {
            } else {
        ...
        this.param_achat_service.PostTypeMatiere(body).pipe(
          catchError((error: HttpErrorResponse) => {
            let errorMessage = '';
  ...
  dataTypeMatiere = new Array<TypeMatiere>();
  // banque: any;
  // listModeReglementRslt = new Array<any>();
  // listModeReglementPushed = new Array<any>();
  GelAllTypeMatiere() {
    this.param_achat_service.GetTypeMatiere().pipe(
      catchError((error: HttpErrorResponse) => {
        let errorMessage = '';
        if (error.error instanceof ErrorEvent) { } else {
          alertifyjs.set('notifier', 'position', 'top-right');
